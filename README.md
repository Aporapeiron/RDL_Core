...
**関係力学言語（RDL: Relational Dynamics Language）** の全ての土台となる、基底措定（BASE）、最低動作仕様（SPEC v2.1）、および対象を操作するための SILN プロトコル（T0/T1）を保持する「公理の聖域」です。
...
│
├── 00_T0_基盤層/
│   ├── T0 基底措定（BASE）.md      (v2.1 相互作用・作用断面版)
│   └── T0 最低動作仕様（SPEC）.md  (v2.1 相互作用・EFP断面運用版)
│
├── 01_T1_SILN操作層/
│   ├── T1_SILN操作_総論.md
│   ├── T1_SILN展開.md
