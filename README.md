...
│   ├── AporapeironとSILN/
│   ├── T1/
│   ├── RIB_B考察_文字に見る作用断面と解釈の境界.md
│   ├── EFP考察_文字に見る作用と解釈の境界.md  (LEGACY / 新文書への案内)
...
T0 / T1 の設計がどのような思考経路から成立したかを保存する層です。`AporapeironとSILN` シリーズ、T1解説、`RIB_B考察` は v2.3 基準へ同期しています。旧 `EFP考察` は過去リンク保持のため LEGACY 案内として残しています。
