...
**関係力学言語（RDL: Relational Dynamics Language）** の全ての土台となる、基底措定（BASE）、最低動作仕様（SPEC v1.2）、および対象を操作するための SILN プロトコル（T0/T1）を保持する「公理の聖域」です。

RDL は、世界そのものを絶対的な真理として直接記述することはしません。有限の境界 $B$ を通じて形成された「解釈と予測（$F$）」のズレ（$E$）を計算し、熱（$H$）の蓄積による再編（$M_\Delta$）を記述するための言語です。
...
├── 00_T0_基盤層/
│   ├── T0 基底措定（BASE）.md
│   └── T0 最低動作仕様（SPEC）.md  (v1.2 二重構造・認識論徹底版)
│
├── 01_T1_SILN操作層/
│   ├── T1_SILN操作_総論.md
│   ├── T1_SILN展開.md
...
- **BASE**: いかなる境界 $B$ にも必ず余剰 $\xi$ が残るという基底措定（Foundational Postulates）。
- **SPEC**: 通常の巡航代謝（$F/F'/E/H/\theta$ 比較ループ）と、限界突破時の高負荷再編相 $M_\Delta$ への移行を定める最低動作仕様。

### 2. T1 SILN操作層 (操作プロトコル)
T0 の力学を用いて、エージェントが対象を解剖・操作・再構成するための運用マニュアル。
- **展開 (Expansion)**: 熱を与えて可能性を限界まで広げる。
- **検査と選別 (Inspection & Selection)**: 目的と許容損失に応じ、T2等の道具を用いて頑健な構造を見極める。
- **再構成 (Reconstruction)**: 新たな $M_B'$ として定着・血肉化させる。
